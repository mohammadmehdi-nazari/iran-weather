...
# تنظیمات Cache (اطلاعات آب‌وهوا برای ۱۰ دقیقه در حافظه نگهداری می‌شود)
app.config["CACHE_TYPE"] = "SimpleCache"
app.config["CACHE_DEFAULT_TIMEOUT"] = 600  # ثانیه (۱۰ دقیقه)
cache = Cache(app)

# لاگ‌گیری ساده برای دیباگ
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# ---------------------------------------------------------------------------
# بارگذاری داده استان‌ها و شهرهای ایران (یک‌بار در زمان اجرا)
# ---------------------------------------------------------------------------
DATA_FILE_PATH = os.path.join(os.path.dirname(__file__), "data", "iran_provinces.json")


def load_iran_data():
    """خواندن فایل JSON استان‌ها و شهرها"""
    try:
...
def classify_weather_color(temp_c, condition_text, precip_mm):
    """
    بر اساس دما، وضعیت و میزان بارش، یک دسته‌بندی رنگی برمی‌گرداند.
    این مقدار در Frontend برای رنگ‌آمیزی استان‌ها استفاده می‌شود.
    """
